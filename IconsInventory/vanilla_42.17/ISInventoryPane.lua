...
function ISInventoryPane:createChildren()
    ...
    self.filterMenu = ISButton:new(self.headerHgt + 1, 0, self.headerHgt, self.headerHgt, "", self,
        ISInventoryPane.onFilterMenu);
    ...
    self.nameHeader = ISResizableButton:new(self.column2, 0, (self.column3 - self.column2), self.headerHgt,
        getText("IGUI_invpanel_Type"), self, ISInventoryPane.sortByName);
    ...
    self.typeHeader = ISResizableButton:new(self.column3 - 1, 0, self.column4 - self.column3 + 1, self.headerHgt,
        getText("IGUI_invpanel_Category"), self, ISInventoryPane.sortByType);
    ...
    self.contextButton1 = ISButton:new(0, 0, btnWid, btnHgt, getText("ContextMenu_Grab"), self, ISInventoryPane
    .onContext);
    ...
    self.contextButton2 = ISButton:new(0, 0, btnWid, btnHgt, getText("IGUI_invpanel_Pack"), self,
        ISInventoryPane.onContext);
    ...
    self.contextButton3 = ISButton:new(0, 0, btnWid, btnHgt, getText("IGUI_invpanel_Pack"), self,
        ISInventoryPane.onContext);
...
function ISInventoryPane:transferItemsByWeight(items, container)
    ...
    for _, item in ipairs(items) do
        ...
        elseif container:getType() == "floor" then
            ...
            ISTimedActionQueue.add(ISInventoryTransferUtil.newInventoryTransferAction(playerObj, item,
                item:getContainer(), container))
        end
    end
end
...
function ISInventoryPane:removeAll(player)
    ...
    local modal = ISModalDialog:new(x, y, width, height, getText("IGUI_ConfirmDeleteItems"), true, self,
        ISInventoryPane.onConfirmDelete, player);
    modal:initialise()
...
function ISInventoryPane:transferAll()
    ...
    if luautils.walkToContainer(self.inventory, self.player) then
        ...
        for i = 0, it:size() - 1 do
            local item = it:get(i);
            local ok = not item:isEquipped() and not item:isItemType(ItemType.KEY_RING) and
            not item:hasTag(ItemTag.KEY_RING) and not hotBar:isInHotbar(item)
...
function ISInventoryPane:updateTooltip()
    ...
    if item then
        if self.toolRender then
            ...
            self.toolRender.anchorBottomLeft = { x = self:getAbsoluteX() + self.column2, y = self:getParent()
            :getAbsoluteY() }
        end
            ...
            if item.items then
                for k, v in ipairs(item.items) do
                    if k ~= 1 and v:getContainer() ~= playerInv then
                        ...
                        ISTimedActionQueue.add(ISInventoryTransferUtil.newInventoryTransferAction(playerObj, v,
                            v:getContainer(), playerInv))
...
function ISInventoryPane:doJoypadExpandCollapse()
    ...
    if not instanceof(self.items[self.joyselection + 1], "InventoryItem") then
        self.collapsed[self.items[self.joyselection + 1].name] = not self.collapsed
        [self.items[self.joyselection + 1].name]
        self:refreshContainer()
    end
end
...
function ISInventoryPane:doContextOnJoypadSelected()
    ...
    if #self.items == 0 then
        local menu = ISInventoryPaneContextMenu.createMenuNoItems(self.player, not isInInv, self:getAbsoluteX() + 64,
            self:getAbsoluteY() + 64)
        if menu then
    ...
    local menu = nil;
    if getCore():getGameMode() == "Tutorial" then
        menu = Tutorial1.createInventoryContextMenu(self.player, isInInv, contextMenuItems, self:getAbsoluteX() + 64,
            self:getAbsoluteY() + 8 + (self.joyselection * self.itemHgt) + self:getYScroll());
    else
        menu = ISInventoryPaneContextMenu.createMenu(self.player, isInInv, contextMenuItems, self:getAbsoluteX() + 64,
            self:getAbsoluteY() + 8 + (self.joyselection * self.itemHgt) + self:getYScroll());
    end
    if menu then
        menu.origin = self.inventoryPage
...
function ISInventoryPane:onRightMouseUp(x, y)
    ...
    if #self.items == 0 then
        local menu = ISInventoryPaneContextMenu.createMenuNoItems(self.player, not isInInv, self:getAbsoluteX() + x,
    ...
    local menu = ISInventoryPaneContextMenu.createMenu(self.player, isInInv, contextMenuItems, self:getAbsoluteX() + x,
        self:getAbsoluteY() + y + self:getYScroll());
    if menu and menu.numOptions > 1 and JoypadState.players[self.player + 1] then
        menu.origin = self.inventoryPage
        menu.mouseOver = 1
    ...
    if self.mouseOverOption ~= 0 and self.items[self.mouseOverOption] ~= nil then
        ...
        if x < self.column2 then
            if not instanceof(self.items[self.mouseOverOption], "InventoryItem") then
                self.collapsed[self.items[self.mouseOverOption].name] = not self.collapsed
                [self.items[self.mouseOverOption].name];
                self:refreshContainer();
                self.selected = {};
                return;
            end
...
function ISInventoryPane:refreshContainer()
    ...
    if expandedItems == 1 then
        self.collapseAll:setVisible(true);
        self.expandAll:setVisible(false);
    else
        self.collapseAll:setVisible(false);
        self.expandAll:setVisible(true);
    end
            ...
            if item:IsFood() and item:getHerbalistType() and item:getHerbalistType() ~= "" then
                if playerObj:isRecipeActuallyKnown("Herbalist") then
                    if item:getHerbalistType() == "Berry" then
                        itemName = (item:getPoisonPower() > 0) and getText("IGUI_PoisonousBerry") or
                        getText("IGUI_Berry")
                    end
                    if item:getHerbalistType() == "Mushroom" then
                        itemName = (item:getPoisonPower() > 0) and getText("IGUI_PoisonousMushroom") or
                        getText("IGUI_Mushroom")
                    end
                else
                    if item:getHerbalistType() == "Berry" then
            ...
            if doIt == true then
                ...
                if self.joyselection ~= nil and self.doController then
                    if self.joyselection == y then
                        self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, self:getWidth() - 1,
                            self.itemHgt, 0.2, 0.2, 1.0, 1.0);
                    end
                end
                ...
                if tex ~= nil then
                    ...
                    if count == 1 then
                        ...
                        if item:isBroken() then
                            self:drawTexture(self.brokenItemIcon, texOffsetX + auxDXY, texOffsetY + auxDXY - 1, 1, 1, 1,
                                1);
                        end
                        if instanceof(item, "Food") and item:isFrozen() then
                            self:drawTexture(self.frozenItemIcon, texOffsetX + auxDXY, texOffsetY + auxDXY - 1, 1, 1, 1,
                                1);
                        end
                        ...
                        if self:isLiteratureRead(player, item) or item:hasBeenSeen(player) or item:hasBeenHeard(player) or player:hasReadMap(item) then
                            self:drawTexture(getTexture("media/ui/Tick_Mark-10.png"), texOffsetX + auxDXY,
                                texOffsetY + auxDXY - 1, 1, 1, 1, 1);
                        end
                        ...
                        if fluidContainer ~= nil and getSandboxOptions():getOptionByName("EnableTaintedWaterText"):getValue() and (not fluidContainer:isEmpty()) and (fluidContainer:contains(Fluid.Bleach) or (fluidContainer:contains(Fluid.TaintedWater) and fluidContainer:getPoisonRatio() > 0.1)) then
                            self:drawTexture(self.poisonIcon, (10 + auxDXY + xoff),
                                (y * self.itemHgt) + self.headerHgt + auxDXY - 1 + yoff, 1, 1, 1, 1);
                        end
                        if item:isFavorite() then
                            self:drawTexture(self.favoriteStar, texOffsetX, texOffsetY + auxDXY, 1, 1, 1, 1);
                        elseif item:isNoRecipes(player) then
                            self:drawTextureScaled(self.noFavoriteRecipeInputStar, texOffsetX, texOffsetY + auxDXY,
                                favoriteRecipeInputStarSize, favoriteRecipeInputStarSize, 1, 1, 1, 1);
                        elseif item:isFavouriteRecipeInput(player) then
                            self:drawTextureScaled(self.favoriteRecipeInputStar, texOffsetX, texOffsetY + auxDXY,
                                favoriteRecipeInputStarSize, favoriteRecipeInputStarSize, 1, 1, 1, 1);
                        end
                    elseif v.count > 2 or (doDragged and count > 1 and self.selected[(y + 1) - (count - 1)] == nil) then
                        ...
                        if player:isEquipped(item) then
                            self:drawTexture(self.equippedItemIcon, texOffsetX + auxDXY + 16, texOffsetY + auxDXY, 1, 1,
                                1, 1);
                        end
                        if not self.hotbar then
                            self.hotbar = getPlayerHotbar(self.player);
                        ...
                        if item:isBroken() then
                            self:drawTexture(self.brokenItemIcon, texOffsetX + auxDXY + 16, texOffsetY + auxDXY - 1, 1, 1,
                                1, 1);
                        end
                        if instanceof(item, "Food") and item:isFrozen() then
                            self:drawTexture(self.frozenItemIcon, texOffsetX + auxDXY + 16, texOffsetY + auxDXY - 1, 1, 1,
                                1, 1);
                        end
                        if self:isLiteratureRead(player, item) or item:hasBeenSeen(player) or item:hasBeenHeard(player) or player:hasReadMap(item) then
                            self:drawTexture(getTexture("media/ui/Tick_Mark-10.png"), texOffsetX + auxDXY,
                        ...
                        if instanceof(item, "Food") and (item:isTainted() and getSandboxOptions():getOptionByName("EnableTaintedWaterText"):getValue()) or player:isKnownPoison(item) then
                            self:drawTexture(self.poisonIcon, 10 + 16 + xoff + auxDXY, texOffsetY + auxDXY - 1, 1, 1, 1,
                                1);
                        end
                        local fluidContainer = item:getFluidContainer() or
                        (item:getWorldItem() and item:getWorldItem():getFluidContainer());
                        ...
                        if item:isFavorite() then
                            self:drawTexture(self.favoriteStar, texOffsetX + auxDXY + 19, texOffsetY + auxDXY - 1, 1, 1,
                                1, 1);
                        elseif item:isNoRecipes(player) then
                            self:drawTextureScaled(self.noFavoriteRecipeInputStar, texOffsetX, texOffsetY + auxDXY,
                                favoriteRecipeInputStarSize, favoriteRecipeInputStarSize, 1, 1, 1, 1);
                ...
                if count == 1 and v.count > 2 then
                    if not doDragged then
                        ...
                        if not self.collapsed[v.name] then
                            self:drawTextureScaled(self.treeexpicon, xPos,
                                (y * self.itemHgt) + self.headerHgt + (self.itemHgt - 15) / 2 + yoff, size, size, 1, 1, 1,
                                0.8)
                        else
                            self:drawTextureScaled(self.treecolicon, xPos,
                                (y * self.itemHgt) + self.headerHgt + (self.itemHgt - 15) / 2 + yoff, size, size, 1, 1, 1,
                                0.8)
                        end
                    end
                ...
                if self.selected[y + 1] ~= nil and not self.highlightItem then -- clicked/dragged item
                    ...
                    elseif false and (((instanceof(item, "Food") or instanceof(item, "DrainableComboItem")) and item:getHeat() ~= 1) or item:getItemHeat() ~= 1) then
                        if (((instanceof(item, "Food") or instanceof(item, "DrainableComboItem")) and item:getHeat() > 1) or item:getItemHeat() > 1) then
                            self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, self.column4,
                                self.itemHgt, 0.5, math.abs(item:getInvHeat()), 0.0, 0.0);
                        else
                            self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, self.column4,
                                self.itemHgt, 0.5, 0.0, 0.0, math.abs(item:getInvHeat()));
                        end
                    else
                        self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, self:getWidth() - 1,
                            self.itemHgt, 0.20, 1.0, 1.0, 1.0);
                    end
                elseif self.mouseOverOption == y + 1 and not self.highlightItem then -- called when you mose over an element
                    if (((instanceof(item, "Food") or instanceof(item, "DrainableComboItem")) and item:getHeat() ~= 1) or item:getItemHeat() ~= 1) then
                        if (((instanceof(item, "Food") or instanceof(item, "DrainableComboItem")) and item:getHeat() > 1) or item:getItemHeat() > 1) then
                            self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, self.column4,
                                self.itemHgt, 0.3, math.abs(item:getInvHeat()), 0.0, 0.0);
                        else
                            self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, self.column4,
                                self.itemHgt, 0.3, 0.0, 0.0, math.abs(item:getInvHeat()));
                        end
                    else
                        self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, self:getWidth() - 1,
                            self.itemHgt, 0.05, 1.0, 1.0, 1.0);
                    end
                else
                    if count == 1 then -- normal background (no selected, no dragging..)
                        ...
                        if self.highlightItem and self.highlightItem == item:getType() then
                            ...
                            self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, self.column4,
                                self.itemHgt, self.blinkAlpha, 1, 1, 1);
                            ...
                            if (((instanceof(item, "Food") or instanceof(item, "DrainableComboItem")) and item:getHeat() ~= 1) or item:getItemHeat() ~= 1) then
                                if (((instanceof(item, "Food") or instanceof(item, "DrainableComboItem")) and item:getHeat() > 1) or item:getItemHeat() > 1) then
                                    if alt then
                                        self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, self.column4,
                                            self.itemHgt, 0.15, math.abs(item:getInvHeat()), 0.0, 0.0);
                                    else
                                        self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, self.column4,
                                            self.itemHgt, 0.2, math.abs(item:getInvHeat()), 0.0, 0.0);
                                    end
                                else
                                    if alt then
                                        self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, self.column4,
                                            self.itemHgt, 0.15, 0.0, 0.0, math.abs(item:getInvHeat()));
                                    else
                                        self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, self.column4,
                                            self.itemHgt, 0.2, 0.0, 0.0, math.abs(item:getInvHeat()));
                                    end
                                end
                            else
                                if alt then
                                    self:drawRect(self.column2 + xoff, (y * self.itemHgt) + self.headerHgt + yoff,
                                        self.column4, self.itemHgt, 0.02, 1.0, 1.0, 1.0);
                                else
                                    self:drawRect(self.column2 + xoff, (y * self.itemHgt) + self.headerHgt + yoff,
                                        self.column4, self.itemHgt, 0.2, 0.0, 0.0, 0.0);
                                end
                            end
                        end
                    else
                ...
                if v.equipped then
                    if not doDragged and not equippedLine and y > 0 then
                        self:drawRect(1, ((y + 1) * self.itemHgt) + self.headerHgt - 1 - self.itemHgt, self.column4, 1,
                            0.2, 1, 1, 1);
                    end
                    equippedLine = true
                end
                ...
                if item:getJobDelta() > 0 and (count > 1 or self.collapsed[v.name]) then
                    ...
                    self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, displayWid * item:getJobDelta(),
                        self.itemHgt, 0.2, 0.4, 1.0, 0.3);
                end

                if self.itemsToHighlight ~= nil and self.itemsToHighlight[item] == true then
                    self:drawRect(1 + xoff, (y * self.itemHgt) + self.headerHgt + yoff, self:getWidth() - 1, self
                    .itemHgt, 0.20, 1.0, 1.0, 1.0);
                end
                ...
                if count == 1 then
                    ...
                    if doDragged and ISMouseDrag.dragging and #ISMouseDrag.dragging > 0 then
                        ...
                        if red then
                            if v.count > 2 then
                                self:drawText(itemName .. " (" .. (v.count - 1) .. ")", self.column2 + 8 + xoff,
                                    (y * self.itemHgt) + self.headerHgt + textDY + yoff, 0.7, 0.0, 0.0, 1.0, self.font);
                            else
                                self:drawText(itemName, self.column2 + 8 + xoff,
                                    (y * self.itemHgt) + self.headerHgt + textDY + yoff, 0.7, 0.0, 0.0, 1.0, self.font);
                            end
                        else
                            if v.count > 2 then
                                self:drawText(itemName .. " (" .. (v.count - 1) .. ")", self.column2 + 8 + xoff,
                                    (y * self.itemHgt) + self.headerHgt + textDY + yoff, textColor.r, textColor.g,
                ...
                if item:getJobDelta() > 0 then
                    if (count > 1 or self.collapsed[v.name]) then
                        if self.dragging == count then
                            self:drawText(item:getJobType(), self.column3 + 8 + xoff,
                                (y * self.itemHgt) + self.headerHgt + textDY + yoff, 0.7, 0.0, 0.0, 1.0, self.font);
                        else
                            self:drawText(item:getJobType(), self.column3 + 8 + xoff,
                                (y * self.itemHgt) + self.headerHgt + textDY + yoff, 0.7, 0.7, 0.7, 1.0, self.font);
                        end
                    end
                else
                    if count == 1 then
                        ...
                        elseif item:getDisplayCategory() then -- display the custom category set in items.txt
                            self:drawText(getText("IGUI_ItemCat_" .. item:getDisplayCategory()), self.column3 + 8 + xoff,
                                (y * self.itemHgt) + self.headerHgt + textDY + yoff, 0.6, 0.6, 0.8, 1.0, self.font);
                        else
                            self:drawText(getText("IGUI_ItemCat_" .. item:getCategory()), self.column3 + 8 + xoff,
                                (y * self.itemHgt) + self.headerHgt + textDY + yoff, 0.6, 0.6, 0.8, 1.0, self.font);
                        end
                    else
...
function ISInventoryPane:drawTextAndProgressBar(text, fraction, xoff, top, fgText, fgBar)
    self:drawText(text, 40 + 30 + xoff, top + (self.itemHgt - self.fontHgt) / 2, fgText.a, fgText.r, fgText.g, fgText.b,
        self.font)
    local textWid = getTextManager():MeasureStringX(self.font, text)
    self:drawProgressBar(40 + math.max(120, 30 + textWid + 20) + xoff, top + (self.itemHgt / 2) - 1,
        self:getProgressBarWidth(), self:getProgressBarHeight(), fraction, fgBar)
end
...
function ISInventoryPane:drawItemDetails(item, y, xoff, yoff, red)
    ...
    elseif instanceof(item, "Food") then
        if item:isIsCookable() and not item:isFrozen() and item:getHeat() > 1.6 then
            ...
            self:drawText(s, 40 + 30 + xoff, top + (self.itemHgt - self.fontHgt) / 2, fgText.a, fgText.r, fgText.g,
                fgText.b, self.font);
            if item:isBurnt() then return end
            local textWid = getTextManager():MeasureStringX(self.font, s)
            self:drawProgressBar(40 + math.max(120, 30 + textWid + 20) + xoff, top + (self.itemHgt / 2) - 1,
                self:getProgressBarWidth(), self:getProgressBarHeight(), f, fgBar);
        elseif item:getFreezingTime() > 0 then
            ...
            if (hunger ~= 0) then
                ...
                self:drawText(item:getName(getSpecificPlayer(self.player)), 40 + 30 + xoff,
                    top + (self.itemHgt - self.fontHgt) / 2, fgText.a, fgText.r, fgText.g, fgText.b, self.font);
            end
        end
    else
        self:drawText(item:getName(getSpecificPlayer(self.player)), 40 + 30 + xoff,
